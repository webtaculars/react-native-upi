      ...
      if(requestCode == UPI_CLIENT_REQUEST) {
        if (mUPIPromise != null) {
          if (resultCode == Activity.RESULT_CANCELED) {
            mUPIPromise.reject("UPI_CLIENT_CANCELLED", "UPI client was cancelled");
          } else if (resultCode == Activity.RESULT_OK) {
            Uri uri = intent.getData();

            if (uri == null) {
              mUPIPromise.reject("NO_RESPONSE", "No respose from UPI client");
            } else {
              mUPIPromise.resolve(uri.toString());
            }
          }
          mUPIPromise = null;
        }
